...
### Added
- **OCR Service**: Extract text from scanned PDFs and images using Tesseract.
- **Searchable PDF**: Convert scanned documents to searchable PDFs.
- **Dependencies**: Added `ocr.go` and `ocr_test.go`.

## [2.2.0] - 2026-02-06

### Added
- **Archive Service**: Convert PDF to PDF/A-1b, 2b, 3b standards for long-term archiving (Enterprise feature).
- **Form Service**: Capability to fill Acrobat Forms (AcroForms) programmatically.
- **Attachment Service**: Add and extract embedded files (XML, Invoice, etc.) from PDF.
- **Enterprise Features**: New `Archive()`, `Form()`, and `Attachment()` methods in main SDK interface.
